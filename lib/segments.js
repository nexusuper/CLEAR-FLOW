...
export function computeSegment({ total_orders, total_spent, last_order }) {
  const ts = new Date(last_order).getTime();
  const daysSinceLast = Number.isFinite(ts) ? (Date.now() - ts) / 86_400_000 : Infinity;
  if (daysSinceLast >= 30 && daysSinceLast < 60) return 'at-risk';
